...
plotI = {imga(:,:,1), imga(:,:,2), imga(:,:,3), imga,...
    maga(:,:,1), maga(:,:,2), maga(:,:,3), magasum,...
    dira(:,:,1), dira(:,:,2), dira(:,:,3), dirasum,...
    imgb(:,:,1), imgb(:,:,2), imgb(:,:,3), imgb,...
    magb(:,:,1), magb(:,:,2), magb(:,:,3), magbsum,...
    dirb(:,:,1), dirb(:,:,2), dirb(:,:,3), dirbsum};
plotTitles = {'ImageA_R', 'ImageA_G', 'ImageA_B', 'ImageA',...
    'MagnitudeA_R', 'MagnitudeA_G', 'MagnitudeA_B', 'MagnitudeA_Sum',...
    'DirectionA_R', 'DirectionA_G', 'DirectionA_B', 'DirectionA_Sum',...
    'ImageB_R', 'ImageB_G', 'ImageB_B', 'ImageB',...
    'MagnitudeB_R', 'MagnitudeB_G', 'MagnitudeB_B', 'MagnitudeB_Sum',...
    'DirectionB_R', 'DirectionB_G', 'DirectionB_B', 'DirectionB_Sum'};
subx = 4;
